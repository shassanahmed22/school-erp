...
export async function PATCH(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.announcement.findFirst({ where: { id: params.id, deletedAt: null } });
  ...
  await logAudit({ userId: guard.payload!.sub, action: "UPDATE", entityType: "Announcement", entityId: announcement.id, newValues: parsed.data, ipAddress, userAgent });
...
export async function DELETE(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  await prisma.announcement.update({ where: { id: params.id }, data: { deletedAt: new Date() } });
  await logAudit({ userId: guard.payload!.sub, action: "DELETE", entityType: "Announcement", entityId: params.id, ipAddress, userAgent });
